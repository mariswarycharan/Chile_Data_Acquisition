...
# Load Excel and CSV files using Vaex for faster operations
market_basket_df = vaex.from_pandas(pd.read_excel(r'Control\Market_Basket.xlsx')) 
...
bulk_downloads_file = vaex.from_csv(r"Output\2024.08.16_15.40.02_2024-1.csv", convert=True)[:10000]  # Selecting first 10,000 rows

# Filtering and extraction (Vectorized in Vaex)
...
# Matching functions with vectorization in Vaex
def find_matches_vectorized(df, choices, score_cutoff=81):
    ...
    comprador_results = df['EspecificacionComprador'].fillna('').apply(get_best_match, progress=True)
    proveedor_results = df['EspecificacionProveedor'].fillna('').apply(get_best_match, progress=True)
...
bulk_downloads_file['Mapped Pactivo from Comprador Brand'] = bulk_downloads_file['Mapped Pactivo from Comprador Brand']
bulk_downloads_file['Mapped Pactivo from Proveedor Brand'] = bulk_downloads_file['Mapped Pactivo from Proveedor Brand']
...
bulk_downloads_file['Dosage_comprador'] = bulk_downloads_file['EspecificacionComprador'].fillna('').apply(extract_dosage, progress=True)
bulk_downloads_file['Dosage_proveedor'] = bulk_downloads_file['EspecificacionProveedor'].fillna('').apply(extract_dosage, progress=True)
...
# Save the final result using Vaex for fast writing
bulk_downloads_file.export_csv('April_Bulk_Presentacion.csv', progress=True, encoding='utf-8-sig')
